var CanvasAnimation = function (id, img, config) {
    ...
    var Point = function (x, y, R, G, B, A, r) {
        ...
        this.drawStep = function (ctx, mouse, center) {
            switch (this.animate) {
                case 'line':
                    // 线性绘图（上、下、左、右）
                    if (this.nY !== this.tY) {
    ...
    this.drawOriginalPoint = function () {
        window.cancelAnimationFrame(this.animationFrame);
        for (var i = 0; i < this.points.length; i++) {
            // 设置为直线动画
     ...
     * 散落到边缘
     */
    this.drawEdgePoint = function (type) {
        window.cancelAnimationFrame(this.animationFrame);
        //this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
        for (var i = 0; i < this.points.length; i++) {
            // 设置为乱序动画
     ...
     * 分散到四周
     */
    this.drawDispersePoint = function () {
        window.cancelAnimationFrame(this.animationFrame);
